...
_NUMBER_METRIC_RE = re.compile(r"\b\d+\.\d+\b|\b\d+\.\d+%|\b\d+%")
...
def validate_executive_narrative(output: str) -> None:
    ...
    if _RAW_METRIC_RE.search(t) or _NUMBER_METRIC_RE.search(t):
...
def _fallback_narrative(executive_brief_json: dict[str, Any]) -> str:
    ...
    dec = str(first.get("decision") or es.get("primary_bet", {}).get("action") or "").strip()
    ...
    impact = str(first.get("business_consequence") or "").strip()
    risk = str(first.get("risk_if_ignored") or "").strip()
    out = str(first.get("outcome") or "").strip()
    why = re.sub(
        r"(?i)the correct move is to",
        "This move",
        str(first.get("decision_rationale") or "").strip(),
    )
    lines = [
        "01 Executive Summary",
    ...
    for i, iss in enumerate(issues[:5], 1):
        problem = str(iss.get("problem") or "").strip()
        bi = str(iss.get("business_consequence") or iss.get("impact") or "").strip()
    ...
    for s in (es.get("execution_plan") or [])[:5]:
        if isinstance(s, dict):
            ...
            acts = [str(a).strip() for a in (s.get("actions") or [])[:2] if str(a).strip()]
            lines.append(f"{focus}: {'; '.join(acts)}")
    lines += [
        "",
        "05 Risks of Inaction",
        ...
        why or "The evidence ties duplicated structure to lost momentum in core buyer journeys.",
    ]
    return "\n".join(lines).strip()
